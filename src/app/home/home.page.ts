...
export class HomePage {
  ...
  save(): void {
    ...
    if (this.editingId) {
      const note = this.notes.find((item) => item.uuid === this.editingId);
      if (note) {
        note.title = title;
  ...
  async sync(): Promise<void> {
    ...
    try {
      ...
      if (pending.length) {
        const syncResponse = await this.request<SyncResponse>(
          '/notes/sync',
          'POST',
          { notes: pending },
        );
      ...
      for (const remote of pulled.notes) {
        const current = stored.find((note) => note.uuid === remote.uuid);

        if (!current || current.syncStatus === 'synced' || remote.updatedAt > current.updatedAt) {
          const index = stored.findIndex((note) => note.uuid === remote.uuid);
          ...
          if (index >= 0) {
            stored[index] = syncedNote;
          } else {
            stored.push(syncedNote);
          }
        }
      }
      ...
      this.loadNotes();
      this.syncMessage = 'Synced successfully.';
    } catch (error) {
  ...
  private persist(): void {
    this.writeStored(this.readStored());
    this.loadNotes();
  }
